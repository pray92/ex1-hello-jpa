...
//                    em.persist(team);
                    ...
                    ;
                    // 다대다
                    // 관계형 데이터베이스는 정규화된 테이블 2로 다대다 관계 표현 불가
                    // 연결 테이블을 추가해서 일대다, 다대일 관계로 풀어내야 함 -> 연결 테이블
                    //
                    // 객체의 경우 컬렉션을 사용해서 객체 2개로 다대다 관계 가능
                    //
                    // 실무에서 쓰면 안됨. 연결 테이블이 연결만하고 끝나는 것이 아님
                    // 주문 시간, 수량 같은 데이터가 들어올 수 있는데 연결 테이블에선 추가가 불가능
                    // 쿼리도 이상하게 나감
                    //
                    // 한계를 극복하기 위해선 연결 테이블도 엔티티도 승격
